...
export default function AdminTourForm() {
  ...
  const confirmSave = () => {
    ...
    const url = isNew ? '/api/tours' : `/api/tours/${id}`;
  ...
  if (loading) return <div style={{ padding: '100px 2rem 4rem', textAlign: 'center' }}>Cargando datos del tour...</div>;
  ...
  return (
    <div style={{ padding: '100px 2rem 4rem', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1 style={{ fontSize: '2rem', marginBottom: '2rem' }}>{isNew ? 'Crear Nuevo Tour' : 'Editar Tour'}</h1>
      ...
      <form onSubmit={handleFormSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '2rem', backgroundColor: 'var(--surface)', padding: '2.5rem', borderRadius: 'var(--radius-lg)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div>
            <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-muted)' }}>Título del Tour</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem' }}>
            ...
            <div>
              <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-muted)' }}>Precio Adultos (COP)</label>
            ...
            <div>
              <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-muted)' }}>Precio Niños (COP)</label>
              <input type="text" name="priceChild" value={formData.priceChild} onChange={handlePriceChange} style={{ width: '100%', padding: '0.8rem', backgroundColor: 'var(--background)', border: '1px solid var(--border)', color: 'var(--text-main)', borderRadius: 'var(--radius-md)' }} placeholder="Opcional" />
            </div>
            <div>
              <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-muted)' }}>Duración</label>
              <input required name="duration" value={formData.duration} onChange={handleChange} placeholder="Ej. 4 horas" style={{ width: '100%', padding: '0.8rem', backgroundColor: 'var(--background)', border: '1px solid var(--border)', color: 'var(--text-main)', borderRadius: 'var(--radius-md)' }} />
            </div>
            <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <label htmlFor="isBestSeller" style={{ cursor: 'pointer', fontWeight: 600 }}>Destacar como "Más vendido"</label>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem', borderTop: '1px solid var(--border)', paddingTop: '2rem' }}>
          <div>
            <h3 style={{ marginBottom: '1rem' }}>¿Qué Incluye?</h3>
        ...
        {/* Itinerary */}
        <div style={{ borderTop: '1px solid var(--border)', paddingTop: '2rem' }}>
          <h3 style={{ marginBottom: '1rem' }}>Itinerario</h3>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000 }}>
          <div style={{ backgroundColor: 'var(--surface)', padding: '2.5rem', borderRadius: 'var(--radius-lg)', maxWidth: '450px', width: '90%', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)', textAlign: 'center', animation: 'slideDown 0.3s ease-out' }}>
            ...
            <h2 style={{ fontSize: '1.5rem', marginBottom: '1rem', color: 'var(--text-main)' }}>
              ¿Estás seguro de {isNew ? 'crear' : 'guardar'} este tour?
            </h2>
            <p style={{ color: 'var(--text-muted)', marginBottom: '2rem', fontSize: '1.1rem' }}>
              El tour <strong>"{formData.title}"</strong> será {isNew ? 'publicado y estará visible en el sitio web principal.' : 'actualizado con los nuevos cambios.'}
            </p>
